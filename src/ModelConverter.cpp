...
{
    buf.reserve(2048);
    // DAE models only use: startTime, dTime, endTime, maxIter, report
    // (maxReps, outToTxt, txtFile are NL-only tokens and cause parse errors in DAE)
    buf.appendFormat(
        "Header:\n"
        "\tstartTime = 0\n"
        "\tdTime = %.6f\n"
        ...
        opts.dTime, opts.endTime, opts.maxIter);
